...
class Collector(object):
    ...
    def collect_from(self, xml_path, default_configuration):
        ...
        try:
            ...
            self.devices[0] = "N/A"
        try:
            self.devices[1] = run.properties['ocl_gpu_deviceName']
        except Exception, e:
            self.devices[1] = "N/A"
  ...
  #          configuration = self.__config_cache[props_key]
       # else:
        ...
        for test in run.tests:
            data_type = module_tests.setdefault((test.shortName(), test.param()), {})

            impl_rec = data_type.setdefault("impl_rec_ipp", {})
            impl_rec[configuration] = ""
            if test.get("impl_rec_ipp") != None:
                impl_rec[configuration] += str(test.get("impl_rec_ipp")) + " "

            impl_rec = data_type.setdefault("impl_rec_ocl", {})
            impl_rec[configuration] = ""
            if test.get("impl_rec_ocl") != None:
                impl_rec[configuration] = test.get("impl_rec_ocl")

            impl_rec = data_type.setdefault("impl_profile", {})
            impl_rec[configuration] = ""
            if test.get("impl_profile") != None:
                impl_rec[configuration] = test.get("impl_profile")

            test_results = data_type.setdefault("res", {})
            new_result = test.get("min") if test.status == 'run' else test.status
...
def main():
    ...
    for sheet_path in args.sheet_dirs:
        ...
        sheet.write(1, 0, 'CPU:', xlwt.easyxf('font: bold true;'))
        sheet.write(1, 1, collector.devices[0], xlwt.easyxf('font: bold true;'))
        sheet.write(2, 0, 'GPU:', xlwt.easyxf('font: bold true;'))
        sheet.write(2, 1, collector.devices[1], xlwt.easyxf('font: bold true;'))

        sheet.row(4).height = 800
        ...
        for config_name in config_names:
            sheet.col(col).width = 4000
            sheet.write(row, col, config_name + "_rec_icv", header_style)
            col += 1
            sheet.col(col).width = 4000
            sheet.write(row, col, config_name + "_rec_ocl", header_style)
            col += 1
            sheet.col(col).width = 4000
            sheet.write(row, col, config_name + "_profile", header_style)
            col += 1
        ...
        for module, tests in sorted(collector.tests.iteritems()):
            for ((test, param), types) in sorted(tests.iteritems()):
                configs = types["res"]
                ...
                for c in config_names:
                    configs = types["impl_rec_ipp"]
                    if c in configs:
                        sheet.write(row, col, configs[c])
                    col += 1

                    configs = types["impl_rec_ocl"]
                    if c in configs:
                        sheet.write(row, col, configs[c])
                    col += 1

                    configs = types["impl_profile"]
                    if c in configs:
                        sheet.write(row, col, configs[c])
                    col += 1
