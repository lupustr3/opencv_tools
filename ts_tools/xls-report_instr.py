...
from testlog_parser_instr import parseLogFile
...
consolas_tree = xlwt.easyxf('font: name Consolas', num_format_str='#0.00')
...
#        else:
        ...
        for test in run.tests:
            data_type = module_tests.setdefault((test.testName(), test.shortName(), test.param()), {})

            instr_first_fun = data_type.setdefault("instr_first_fun", test.firstFunction)

            instr_rec = data_type.setdefault("instr_trace_ipp", {})
            instr_ipp_f_rec = data_type.setdefault("instr_trace_ipp_f_ipp", {})
            instr_ipp_weight_rec = data_type.setdefault("instr_total_ipp_weight", {})
            instr_rec[configuration] = ""
            instr_ipp_f_rec[configuration] = ""
            instr_ipp_weight_rec[configuration] = ""
            if test.get("functions_hierarchy") != None:
                instr_rec[configuration] += test.showTree() + " "
                instr_ipp_f_rec[configuration] += test.showIPPFuncs() + " "

            if test.get("total_ipp_weight") != None:
                instr_ipp_weight_rec[configuration] += test.showtotalIPPWeight()

            test_results = data_type.setdefault("res", {})
...
def main():
    ...
    try:
        f = open('C:\\OpenCV\\opencv\\3rdparty\\ippicv\\unpack\\ippicv_win\\include\\ippicv_redefs.h', 'r')
        for line in f:
            indx_ippicv = line.find(" ippicv")
            if indx_ippicv > -1:
                allICVFunctionsList.append(line[indx_ippicv+1:line.rfind("\n")].decode('ascii'))

    except IOError as ioe:
            analyseICVHeader = 0;
            logging.debug('no ippicv_redefs.h found')

    testsICVFunctionsList = []
    ...
    for sheet_path in args.sheet_dirs:
        ...
        for config_name in config_names:
            ...
            sheet.col(col+1).width = 7500
            sheet.write(row, col+1,config_name + "\nIPP functions",header_style)
            sheet.col(col+2).width = 4000
            sheet.write(row, col+2,config_name + "\nIPP Weight",header_style)
            col += 3
        ...
        for module, tests in sorted(collector.tests.iteritems()):
            for ((test, orig_tn, param), types) in sorted(tests.iteritems()):
                configs = types["res"]
                col = 0



                sheet.write(row, col, module, module_styles.get(module, xlwt.Style.default_style))
                col += 1
                sheet.write(row, col, types["instr_first_fun"])
                col += 1
                sheet.write(row, col, test)
                col += 1
                sheet.write(row, col, orig_tn)
                col += 1
                ...
                #col += 1 # blank column
                for c in config_names:
                    configs = types["instr_trace_ipp"]
                    if c in configs:
                        if configs[c] != "":
                            sheet.write(row, col, configs[c], consolas_tree)
                            col += 1
                    configs = types["instr_trace_ipp_f_ipp"]
                    if c in configs:
                        if configs[c] != "":
                            ippfunction = ""
                            ippfunction = configs[c]
                            if ippfunction != "":
                                tmp = ippfunction.split("; ")
                                for i in tmp:
                                    if i == " ":
                                        tmp.remove(i)
                                testsICVFunctionsList.extend(tmp)
                            sheet.write(row, col, configs[c], consolas_tree)
                            col += 1
                    configs = types["instr_total_ipp_weight"]
                    if c in configs:
                        if configs[c] != "":
                            sheet.write(row, col, configs[c])
                            col += 1

                row += 1
                if row % 1000 == 0: sheet.flush_row_data()

    if analyseICVHeader == 1:
        f1 = open('ippFunctionsList-NOT-Covered.txt', 'w')
        f2 = open('ippFunctionsList-Covered.txt', 'w')
        for a in allICVFunctionsList:
            count = testsICVFunctionsList.count(a)
            if count == 0:
                f1.write(a + '\n')
            else:
                f2.write(a + '\n')
